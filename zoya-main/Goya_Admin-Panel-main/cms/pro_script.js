...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    // Gather form data
    const title = document.getElementById("title").value;
    const description = document.getElementById("content1").innerHTML; // Use the content from the editor
    const media = document.getElementById("file-input").files; // Assuming media is uploaded
    const category = document.getElementById("category").value;
    const denominations = Array.from(
      document.querySelectorAll('input[name="denominations[]"]')
    ...
    const product = {
      title,
      description,
      media: Array.from(media).map((file) => file.name), // Just storing file names as example
      category,
      denominations,
      status,
      productType,
    ...
    // Redirect to manage_products.html
    window.location.href = "manage_products.html";
  });
});
document.addEventListener("DOMContentLoaded", function () {
  ...
  products.forEach((product) => {
    ...
    row.setAttribute(
      "data-url",
      "edit_product.html?id=" + products.indexOf(product)
    ); // Example URL
    ...
    row.innerHTML = `
            ...
            <td class="img_con"><img src="images/dummy_product.jpg" /></td>
